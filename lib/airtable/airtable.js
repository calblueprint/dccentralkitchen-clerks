  ...
  return new Promise(function(resolve, reject) {
    ...
    base(table).create([{ fields: transformedRecord }], function(err, records) {
      ...
      if (records.length !== expectedLen) {
        reject(new Error(`${records.length} records returned from creating 1 record. Expected: ${expectedLen}`));
        return;
      }
    ...
    base(table)
      ...
      .eachPage(
        function page(records, fetchNextPage) {
          ...
          resolve(records.map(record => fromAirtableFormat(record.fields, table)));
