...
const fromAirtableFormat = (record, table) => {
  ...
  if (!columns) {
    throw new Error(`Error converting record from Airtable. Could not find table: ${table}`);
  }
...
const toAirtableFormat = (record, table) => {
  const columns = Columns[table];
  if (!columns) {
    throw new Error(`Error converting record for Airtable. Could not find table: ${table}`);
  }
...
*/
function getRecordsByAttribute(table, fieldType, fieldValue, filterByFormula = '', sort = []) {
  return base(table)
    .select({
      view: VIEW,
