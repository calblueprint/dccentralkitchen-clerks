...
export const createManyCustomers = async (records) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    ...
    if (subset.length > 0) createPromises.push(createRecords(Tables.Customers, subset));
  }
...
export const createManyClerks = async (records) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = records.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) createPromises.push(createRecords(Tables.Clerks, subset));
  }
  return Promise.all(createPromises);
};
...
export const createManyStores = async (records) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = records.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) createPromises.push(createRecords(Tables.Stores, subset));
  }
  return Promise.all(createPromises);
};
...
export const createManyProducts = async (records) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = records.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) createPromises.push(createRecords(Tables.Products, subset));
  }
  return Promise.all(createPromises);
};
...
export const createManyResources = async (records) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = records.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) createPromises.push(createRecords(Tables.Resources, subset));
  }
  return Promise.all(createPromises);
};
...
export const createManyTransactions = async (records) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = records.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) createPromises.push(createRecords(Tables.Transactions, subset));
  }
  return Promise.all(createPromises);
};
...
export const createManyLineItems = async (records) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = records.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) createPromises.push(createRecords(Tables.LineItems, subset));
  }
  return Promise.all(createPromises);
};
...
export const createManyPushTokens = async (records) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = records.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) createPromises.push(createRecords(Tables.PushTokens, subset));
  }
  return Promise.all(createPromises);
};
...
export const createManyNews = async (records) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = records.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) createPromises.push(createRecords(Tables.News, subset));
  }
  return Promise.all(createPromises);
};
...
export const createManyRecipes = async (records) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = records.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) createPromises.push(createRecords(Tables.Recipes, subset));
  }
  return Promise.all(createPromises);
};
...
export const getCustomersByPhoneNumber = async (value, filterByFormula = '', sort = []) => {
...
export const getResourcesByIds = async (ids, filterByFormula = '', sort = []) => {
  let formula = `OR(${ids.reduce((f, id) => `${f} {ID}='${id}',`, '')} 1 < 0)`;
  formula = filterByFormula ? `AND(${filterByFormula}, ${formula})` : formula;
...
export const getTransactionsByIds = async (ids, filterByFormula = '', sort = []) => {
  let formula = `OR(${ids.reduce((f, id) => `${f} {ID}='${id}',`, '')} 1 < 0)`;
  formula = filterByFormula ? `AND(${filterByFormula}, ${formula})` : formula;
...
export const getLineItemsByIds = async (ids, filterByFormula = '', sort = []) => {
  let formula = `OR(${ids.reduce((f, id) => `${f} {ID}='${id}',`, '')} 1 < 0)`;
  formula = filterByFormula ? `AND(${filterByFormula}, ${formula})` : formula;
...
export const getPushTokensByIds = async (ids, filterByFormula = '', sort = []) => {
  let formula = `OR(${ids.reduce((f, id) => `${f} {ID}='${id}',`, '')} 1 < 0)`;
  formula = filterByFormula ? `AND(${filterByFormula}, ${formula})` : formula;
...
export const updateManyCustomers = async (recordUpdates) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    ...
    if (subset.length > 0) updatePromises.push(updateRecords(Tables.Customers, subset));
  }
...
export const updateManyClerks = async (recordUpdates) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = recordUpdates.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) updatePromises.push(updateRecords(Tables.Clerks, subset));
  }
  return Promise.all(updatePromises);
};
...
export const updateManyStores = async (recordUpdates) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = recordUpdates.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) updatePromises.push(updateRecords(Tables.Stores, subset));
  }
  return Promise.all(updatePromises);
};
...
export const updateManyProducts = async (recordUpdates) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = recordUpdates.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) updatePromises.push(updateRecords(Tables.Products, subset));
  }
  return Promise.all(updatePromises);
};
...
export const updateManyResources = async (recordUpdates) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = recordUpdates.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) updatePromises.push(updateRecords(Tables.Resources, subset));
  }
  return Promise.all(updatePromises);
};
...
export const updateManyTransactions = async (recordUpdates) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = recordUpdates.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) updatePromises.push(updateRecords(Tables.Transactions, subset));
  }
  return Promise.all(updatePromises);
};
...
export const updateManyLineItems = async (recordUpdates) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = recordUpdates.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) updatePromises.push(updateRecords(Tables.LineItems, subset));
  }
  return Promise.all(updatePromises);
};
...
export const updateManyPushTokens = async (recordUpdates) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = recordUpdates.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) updatePromises.push(updateRecords(Tables.PushTokens, subset));
  }
  return Promise.all(updatePromises);
};
...
export const updateManyNews = async (recordUpdates) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = recordUpdates.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) updatePromises.push(updateRecords(Tables.News, subset));
  }
  return Promise.all(updatePromises);
};
...
export const updateManyRecipes = async (recordUpdates) => {
  ...
  for (let i = 0; i < numCalls; i += 1) {
    const subset = recordUpdates.slice(i * 10, (i + 1) * 10);
    if (subset.length > 0) updatePromises.push(updateRecords(Tables.Recipes, subset));
  }
  return Promise.all(updatePromises);
};
