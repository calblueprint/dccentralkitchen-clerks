...
function createProductData(record) {
  return { ...record, cartCount: 0, imageUrl: record.image ? record.image[0].url : null };
}
...
export async function addTransaction(customer, cart, currentPoints, totalPoints, totalPrice, rewardsApplied) {
...
export function calculateLineItemPrice(product) {
  return (product.customerCost * product.cartCount).toFixed(2);
}

// TODO: change back to 5
// Given that there are always 5 columns of products, return the number of rows in the product container.
export function getNumRowsProducts(products) {
  return Math.floor(products.length / 3);
}
